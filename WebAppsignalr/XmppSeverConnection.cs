...
namespace WebAppsignalr
{
    public class XmppSeverConnection
    {
        ...
        public const string XmppDomain = "192.168.0.101";
            ...
            if (pres.To == null || pres.To.Equals(XmppDomain, new FullJidComparer()))
            {
                ...
                if (pres.Type == PresenceType.Available)
                {
                    if (IsBinded && !InitialPresence)
                    {
                        ...
                        int j = Global.ServerConnections.Count();
                        ...
                        {
                            // but not myself
                            if (contact1[i].Equals(Jid.User))
                                continue;

                            var jid = new Jid(contact1[i] + "@" + XmppDomain);
                            //var jid = new Jid(Jid.User + "@" + XmppDomain);
                ...
                */
                ...
                for (int i = 0; i < contact.Length; i++)
                {
                    ...
                    var jid = new Jid(contact[i] + "@" + XmppDomain);
                    var con = Global.ServerConnections.FirstOrDefault(sc => sc.Jid.Equals(jid, new BareJidComparer()));
                    if (con != null)
                        con.Send(pres);
                }
